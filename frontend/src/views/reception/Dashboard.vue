          ...
          <el-form-item label="到店预订">
            <el-select v-model="form.booking_id" placeholder="选择待到店的预订" style="width: 300px">
              <el-option v-for="a in arrivals" :key="a.id" :value="a.id"
                         :label="`#${a.id} ${a.customer_name} · ${a.room_type_name} · ${a.check_in_date}`" />
            </el-select>
          </el-form-item>
        </template>
        <template v-else>
          <el-form-item label="客人姓名">
            <el-input v-model="form.guest_name" placeholder="姓名" style="width: 130px" />
          </el-form-item>
          <el-form-item label="手机号">
            <el-input v-model="form.guest_phone" placeholder="手机号" style="width: 150px" />
          </el-form-item>
          <el-form-item label="房型">
            <el-select v-model="form.room_type_id" placeholder="选择房型" style="width: 150px">
...
async function load() {
  await Promise.all([loadArrivals(), loadStats()])
...
function pickBooking(row) {
  ...
  if (!form.expected_check_out) form.expected_check_out = row.check_out_date
}

async function checkIn() {
  const payload = { expected_check_out: form.expected_check_out }
  if (form.mode === 'booking') {
    if (!form.booking_id) return ElMessage.warning('请选择到店预订')
